...
def send_latest_data():
    ...
    try:
        ...
        if result:
            ...
            print(f"\U0001F4E4 Đã gửi dữ liệu mới nhất lên MQTT topic {MQTT_TOPIC_GUI}: {mqtt_payload}")
    except Exception as e:
...
def on_message(client, userdata, msg):
    try:
        ...
        print(f"\U0001F4E5 Nhận dữ liệu từ ESP: {data}")

        # Kiểm tra nếu nhận được yêu cầu gửi dữ liệu mới nhất
        if "bat_dau" in data and data["bat_dau"] is True:
            print("\U0001F501 Nhận yêu cầu gửi dữ liệu mới nhất lên MQTT!")
            send_latest_data()
            return

        # Danh sách key cần có để lưu vào MySQL
